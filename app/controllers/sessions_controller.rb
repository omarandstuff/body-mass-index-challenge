class SessionsController < ApplicationController
  ...
  def login
    session = Sessions::Retrive email: params[:email], password: params[:password], token: request.headers[:HTTP_CSRF_TOKEN]
  ...
  def register
    ...
    if user.save
      session = Sessions::Retrive email: user.email, password: user.password, token: nil
