class SessionsController < ApplicationController
  ...
  def login
    session = Sessions::Retrieve.new(email: params[:email], password: params[:password], token: request.headers[:HTTP_CSRF_TOKEN]).preocess
  ...
  def register
    ...
    if user.save
      session = Sessions::Retrieve.new(email: user.email, password: user.password, token: nil).preocess
